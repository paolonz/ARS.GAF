/*
 ...
 <appSettings>
    <add key="sso.check.url" value="http://cohesion.regione.marche.it/SSO/Check.aspx"/>
    <add key="sso.webCheckSessionSSO" value="https://cohesion.regione.marche.it/SSO/webCheckSessionSSO.aspx"/>
 ...
 <appSettings>
    <add key="sso.check.url" value="https://cohesion.regione.marche.it/SPManager/WAYF.aspx"/>
    <add key="sso.webCheckSessionSSO" value="https://cohesion.regione.marche.it/SPManager/webCheckSessionSSO.aspx"/>
    ...
    <add key="site.URLLogout" value="../Logout.aspx?ReturnUrl=https%3A%2F%2Fcohesion.regione.marche.it%2FSPManager%2FLogout.aspx"/> <!-- Inserire l'url della propria pagina di logout relativo alla posizione della pagina protetta; non modificare il valore di ReturnUrl -->
 </appSettings>

 */

namespace Cohesion.SSO
{
    ...
    public class CohesionSSO
    {
        ...
        {
            if (String.IsNullOrEmpty(SSOCheckUrl) || String.IsNullOrEmpty(webCheckSessionSSOUrl))
            {
                response.Clear();
            ...
            if (request.Params["auth"] == null)
            {
                ...
                response.Redirect(SSOCheckUrl + "?auth=" + System.Web.HttpUtility.UrlEncode(auth));
            }
            else
            {
